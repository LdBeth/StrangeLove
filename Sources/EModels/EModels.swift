...
/// and invokes us as:
///   EModels [-config FILE] [-f DBFILE] mark
///   EModels [-config FILE] [-f DBFILE] add -v -spam
///   EModels [-config FILE] [-f DBFILE] add -v -good
...
struct EModels {
    static func main() async {
        ...
        guard let args = Arguments.parse(argv) else {
            FileHandle.standardError.write(Data(
                "Usage: EModels [-config FILE] [-f DBFILE] (mark | add -v (-spam|-good) | distill [--model NAME])\n".utf8))
            exit(2)
        }
    ...
    private static func emitMarked(rawMessage: Data, isSpam: Bool) {
        let header = isSpam ? "X-Spam: yes; EModels\n" : "X-Spam: no; EModels\n"
